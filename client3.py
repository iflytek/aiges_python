...
def _initialize_worker(server_address):
    ...
    _worker_stub_singleton = aiges_inner_pb2_grpc.WrapperServiceStub(
...
def _run_worker_query(primality_candidate):
    ...
    ret = _worker_stub_singleton.wrapperInit(aiges_inner_pb2.InitRequest(config={"ccc": "ddd"}))
...
def main():
    ...
    parser.add_argument('server_address', required=False, default="localhost:61526",
                        help='The address of the server (e.g. localhost:50051)')
    args = parser.parse_args()
    primes = _calculate_primes("localhost:61526")
    print(primes)
